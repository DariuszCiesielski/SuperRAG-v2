  ...
  const renderHighlightedContent = () => {
    return lines.map((line, index) => {
      ...
      return (
        ...
        >
          <span className={isHighlighted ? 'font-medium' : ''} style={{ color: 'var(--text-primary)' }}>{line}</span>
        </div>
      );
    });
  };
    ...
    <div className={`flex flex-col h-full overflow-hidden ${className}`}>
      ...
      <div className="p-4 flex-shrink-0" style={{ borderBottom: '1px solid var(--border-primary)' }}>
        <div className="flex items-center space-x-2 mb-2">
          ...
          <span className="font-medium truncate" style={{ color: 'var(--text-primary)' }}>{citation.source_title}</span>
        </div>
      </div>
          ...
          <Accordion type="single" value={accordionValue} onValueChange={setAccordionValue} collapsible>
            <AccordionItem value="guide" className="border-0">
              <AccordionTrigger
                ...
                chevronColor="var(--info)"
              >
                <div className="flex items-center space-x-2">
                  <svg xmlns="http://www.w3.org/2000/svg" height="16px" viewBox="0 -960 960 960" width="16px" style={{ fill: 'var(--info)' }}>
              ...
              <AccordionContent className="px-4 pb-4">
                <div className="text-sm space-y-4" style={{ color: 'var(--text-secondary)' }}>
                  ...
                  {citation.source_type === 'website' && sourceUrl && (
                    <div>
                      <h4 className="font-medium mb-2" style={{ color: 'var(--text-primary)' }}>URL</h4>
                      <a
                        href={sourceUrl}
                        target="_blank"
